...
def sign(message, selector, domain, privkey, identity=None, canonicalize=(Simple, Simple), include_headers=None, length=False, debuglog=None):
    ...
    if debuglog is not None:
        print ("sign headers:", sign_headers + [("DKIM-Signature", " "+"; ".join("%s=%s" % x for x in sigfields))],file=debuglog)
    if HaveSHA256:
        h = hashlib.sha256()
    else:
        h = sha.sha()
...
def verify(message, debuglog=None):
    ...
    if debuglog is not None:
        print ("a:{}".format(a),file = debuglog)
    sig = {}
    for x in a:
        if x:
    ...
    if 'i' in sig and (not sig['i'].endswith(sig['d']) or sig['i'][-len(sig['d'])-1] not in "@."):
        if debuglog is not None:
            print ("i= domain is not a subdomain of d= (i=%s d=%d)" % (sig['i'], sig['d']), file = debuglog)
        return False
    if 'l' in sig and re.match(r"\d{,76}$", sig['l']) is None:
        if debuglog is not None:
            print ("l= value is not a decimal integer (%s)" % sig['l'], file = debuglog)
        return False
    ...
    if 'x' in sig:
        if re.match(r"\d+$", sig['x']) is None:
            if debuglog is not None:
                print ("x= value is not a decimal integer (%s)" % sig['x'], file = debuglog)
            return False
        if int(sig['x']) < int(sig['t']):
            if debuglog is not None:
                print ("x= value is less than t= value (x=%s t=%s)" % (sig['x'], sig['t']), file = debuglog)
            return False

    m = re.match("(\w+)(?:/(\w+))?$", sig['c'])
    if m is None:
        if debuglog is not None:
            print ("c= value is not in format method/method (%s)" % sig['c'], file = debuglog)
        return False
    ...
    if sig['a'] == "rsa-sha1":
        ...
        if not HaveSHA256:
            if debuglog is not None:
                print ("Unsupported signature algorithm (%s)" % sig['a'], file = debuglog)
            return False
    ...
    if bodyhash != base64.b64decode(re.sub(r"\s+", "", sig['bh'])):
        if debuglog is not None:
            print ("body hash mismatch (got %s, expected %s)" % (base64.b64encode(bodyhash), sig['bh']), file = debuglog)
        return False
    ...
    d = h.digest()
    if debuglog is not None:
        print ("verify digest:", " ".join("%02x" % ord(x) for x in d), file = debuglog)
